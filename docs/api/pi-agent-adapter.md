...
首版通过独立子进程接入 `pi` agent，让配套 Electron 主进程可以在保留既有 preload IPC、persona、memory、world、planner、swarm 体系的前提下，跑通基础聊天、文件操作和 skills 能力。

## 首版范围

- Electron main process 启动并管理 `pi` agent 子进程。
- 桌面 UI 继续通过现有 `agent:chat`、`agent:stream`、`agent:abort` 等 IPC 入口与主进程交互。
- 主进程中的适配层负责把桌面 IPC 请求转成子进程输入事件，并把子进程输出事件转成桌面端现有流式事件。
...
- 可通过 `CHENYUAN_PI_AGENT_CLI` 或 `PI_AGENT_CLI` 指定 `pi` CLI 入口。
- 可通过 `CHENYUAN_PI_AGENT_NODE` 指定运行 agent 子进程的 Node 可执行文件。
- 可通过 `CHENYUAN_PI_AGENT_PROVIDER` 或 `PI_AGENT_PROVIDER` 覆盖 `pi` provider；未设置时默认使用 `deepseek`。
- 可通过 `CHENYUAN_PI_AGENT_MODEL` 或 `PI_AGENT_MODEL` 覆盖默认模型；未设置时默认使用 `deepseek-v4-pro`。
- 桌面 UI 当前选中的模型不会覆盖 `pi` agent 默认模型，避免既有桌面端模型选择影响首版 `pi` 底层验证。
