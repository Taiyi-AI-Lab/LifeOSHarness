...
陈远是仓库自带的 example preset，用来展示结构化 Agent Pack 的能力。他是 2035 年 35 岁的 UBI 公民，曾五次被自动化替代，并围绕尊严、机器人权利和活着的意义形成性格底色；你可以通过 `POST /packs` 创建自己的 Pack。
...
|------|---------|------|-------------|--------------|---------------|------|
| 陈远 | `chenyuan` | 内置 preset | 《2035》主角设定启发的 LifeOS 角色 | 35 岁 UBI 公民，失业五次的底层程序员，围绕技术替代、尊严、机器人权利和意义感展开 | persona、emotion、memory、world_facts、proactive | [chenyuan.py](lifeostomanyagent/server/presets/chenyuan.py) |
...
Web Console 位于 `web/`，通过 Vite proxy 连接 FastAPI server。
...
打开 `http://127.0.0.1:5173`，使用后端配置的同一个 `X-API-Key`。
...
|------|------|------|
| GET | `/health` | 健康检查 |
| POST | `/packs/presets/chenyuan` | 安装/刷新 陈远示例 preset |
...
| GET | `/inspector/worlds/{world_id}/state` | 查看 Web Console 所需的聚合 runtime state |
